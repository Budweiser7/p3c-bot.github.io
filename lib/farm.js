    ...
    farmContract.myCrop.call(function (err, result) {
        ...
        if ((onboard == true) && (err || (result == '0x0000000000000000000000000000000000000000'))) {
            alertify.confirm('Welcome to P3C',
                `
                <p class="agreement">
                P3C is an autonomous smart contract. You can read it <a target="_blank" href="http://etherhub.io/addr/0xae5433263a626f397fed88421cc85ffd22bbc8dd">here</a>. By clicking OK, you acknowledge that the contract will be run exactly as written and that code is law.
                </p>
                ...
                function () {
                    alertify.error('View mode.')
                });
        }
        if (!err) {
            myCropAddress = result;
            $("#myCropAddress").replaceWith("<b>" + myCropAddress + "</b>")
            $("#masternodeLink").replaceWith('<a id="masternodeLink" href="/?masternode=' + myCropAddress + '">https://p3c.io/index.html?masternode=' + myCropAddress + '</a>')
        }
    });
}
...
function getMyCropDividends() {
    farmContract.myCropDividends.call(function (err, result) {
        if (!err) {
            ...
            $("#myCropDividends").replaceWith("<b id='myCropDividends'>" + web3.fromWei(myCropDividends) + "</b>")
        }
    });
}
...
function reinvestFromCrop() {
    farmContract.myCrop.call(function (err, cropAddress) {
        cropAbi.at(cropAddress).reinvest.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    alertify.success("Reinvested P3C")
                    console.log(result);
                } else {
                    console.log(error);
                }
...
function withdrawFromCrop() {
    farmContract.myCrop.call(function (err, cropAddress) {
        cropAbi.at(cropAddress).withdraw.sendTransaction({
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    alertify.success("Withdrawing Dividends")
                    console.log(result);
                } else {
                    console.log(error);
                }
...
function transferFromCrop(destination, amountToTransfer) {
    ...
    farmContract.myCrop.call(function (err, cropAddress) {
        cropAbi.at(cropAddress).transfer.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    alertify.success("Transfering " + amountToTransfer + " P3C to " + destination)
                    console.log(result);
                } else {
                    console.log(error);
                }
